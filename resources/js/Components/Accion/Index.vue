<template>
  <div>
    <div style="margin-bottom:50px!important" class="d-flex align-items-center font-weight-bold my-2 mb-4" bis_skin_checked="1">
        ...
        <a href="/" style="font-size: 13!important" class="opacity-75 hover-opacity-100" bis_skin_checked="1">Acciones</a>
        <!--end::Item-->
    </div>
    <h3 class="mt-4">Tipo Acciones</h3>
    <br />
    <div class="card card-custom">
      <div class="card-header flex-wrap py-5">
        <div class="card-title">
          <h3 class="card-label">
            Datos de los acciones asociado
...
export default {
  ...
  methods: {
    ...
    getCliente(id) {
      axios.get("/api/accion/" + id).then((response) => {
    ...
    updateEstado() {
      this.clientFormLoading = true;
      axios
        .put("/api/accion/" + this.idCliente, this.form)
        .then((response) => {
          $("#createModal").modal("hide");
          this.clientFormLoading = false;
          this.registro = false;
          this.$swal({ icon: "success", title: "Accion Actualizada" });
          this.getEstados();
        })
        .catch((error) => {
    ...
    crearEstado() {
      this.clientFormLoading = true;
      axios
        .post("/api/accion", this.form)
        .then((response) => {
          $("#createModal").modal("hide");
          this.clientFormLoading = false;
           this.registro = false;
          this.$swal({ icon: "success", title: "Accion Guardada" });
