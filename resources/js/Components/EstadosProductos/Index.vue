<template>
  <div>
    ...
    <h3 class="mt-4">Tipo Estados Productos</h3>
    <br />
    <div class="card card-custom">
      <div class="card-header flex-wrap py-5">
        <div class="card-title">
          <h3 class="card-label">
            Datos de los estados de productos Asociado
...
export default {
  ...
  methods: {
    getEstados(page = 1,table = "name") {
      ...
      axios.get("/api/estados-productos?page=" + page + "&paginate=" + this.paginate + "&q=" + this.search + "&sort=" + sort + "&table=" + table).then((response) => {
    ...
    getCliente(id) {
      axios.get("/api/estados-productos/" + id).then((response) => {
    ...
    delete_cliente(id, name) {
      this.$swal({
        ...
        if (result.value) {
          axios
            .delete("/api/estados-productos/" + id+"?id_usuario="+user.id)
            .then((response) => {
              /*
              this.$swal({
                icon: "success",
    ...
    updateEstado() {
      ...
      console.log(user.id)
      axios
        .put("/api/estados-productos/" + this.idCliente+"?id_usuario="+user.id, this.form)
        .then((response) => {
    ...
    crearEstado() {
      this.clientFormLoading = true;
      axios
        .post("/api/estados-productos"+"?id_usuario="+user.id, this.form)
        .then((response) => {
          $("#createModal").modal("hide");
          this.clientFormLoading = false;
           this.registro = false;
