<template>
  <div>
    ...
    <h3 class="mt-4">Tipo de Clientes</h3>
    <br />
    <div class="card card-custom">
      <div class="card-header flex-wrap py-5">
        <div class="card-title">
          <h3 class="card-label">
            Datos de los Clientes Asociado
               ...
               <div class="table-responsive" style="margin-top:15px!important">
                   <div class="alert alert-custom alert-warning"
                    ...
                    <div class="alert-text">
                        No hay tipos de tipo clientes registrados 
                    </div>
                    </div>
        ...
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">
              {{ editMode ? "Agregar" : "Editar" }} tipo de Cliente
            </h5>
            <button
              type="button"
              class="close"
...
export default {
  ...
  methods: {
    getClientes(page = 1,table = "name") {
      ...
      axios.get("/api/tipo-clientes?page=" + page + "&paginate=" + this.paginate + "&q=" + this.search + "&sort=" + sort + "&table=" + table).then((response) => {
    ...
    getCliente(id) {
      axios.get("/api/tipo-clientes/" + id).then((response) => {
    ...
    delete_cliente(id, name) {
      this.$swal({
        ...
        if (result.value) {
          axios
            .delete("/api/tipo-clientes/" + id+"?id_usuario="+user.id)
            .then((response) => {
              /*
              this.$swal({
                icon: "success",
    ...
    updateCliente() {
      this.clientFormLoading = true;
      axios
        .put("/api/tipo-clientes/" + this.idCliente+"?id_usuario="+user.id, this.form)
        .then((response) => {
    ...
    crearCliente() {
      this.clientFormLoading = true;
      axios
        .post("/api/tipo-clientes"+"?id_usuario="+user.id, this.form)
        .then((response) => {
          $("#createModal").modal("hide");
          this.clientFormLoading = false;
          //this.$swal({ icon: "success", title: "Tipo de Cliente Guardado" });
          toastr.success('Tipo de Cliente Guardado');
          this.getClientes();
        })
        .catch((error) => {
          if (error.response.status === 422) {
