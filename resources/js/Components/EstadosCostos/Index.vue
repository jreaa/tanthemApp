...
export default {
  ...
  methods: {
    getEstados(page = 1,table = "name") {
      ...
      axios.get("/api/estados_costos?page=" + page + "&paginate=" + this.paginate + "&q=" + this.search + "&sort=" + sort + "&table=" + table).then((response) => {
    ...
    getCliente(id) {
      axios.get("/api/estados_costos/" + id).then((response) => {
    ...
    delete_cliente(id, name) {
      this.$swal({
        ...
        if (result.value) {
          axios
            .delete("/api/estados_costos/" + id+"?id_usuario="+user.id)
            .then((response) => {
              /*
              this.$swal({
                icon: "success",
    ...
    updateEstado() {
      ...
      console.log(user.id)
      axios
        .put("/api/estados_costos/" + this.idCliente+"?id_usuario="+user.id, this.form)
        .then((response) => {
    ...
    crearEstado() {
      this.clientFormLoading = true;
      axios
        .post("/api/estados_costos"+"?id_usuario="+user.id, this.form)
        .then((response) => {
          $("#createModal").modal("hide");
          this.clientFormLoading = false;
           this.registro = false;
