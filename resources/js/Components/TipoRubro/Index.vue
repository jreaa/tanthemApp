<template>
    <div>
      <div style="margin-bottom:50px!important" class="d-flex align-items-center font-weight-bold my-2 mb-4" bis_skin_checked="1">
          ...
          <a href="/" style="font-size: 13!important" class="opacity-75 hover-opacity-100" bis_skin_checked="1">Tipo de Clientes</a>
          <!--end::Item-->
      </div>
      <h3 class="mt-4">Tipo de Rubros</h3>
      <br />
      <div class="card card-custom">
        <div class="card-header flex-wrap py-5">
          <div class="card-title">
            <h3 class="card-label">
              Datos de los rubros asociado
      ...
      margin-left: auto!important;"
                      ...
                      <div class="alert-text">
                          No hay tipos de tipo rubros registrados 
                      </div>
                      </div>
                      ...
                      >
                          ...
                          <tr class="even" v-for="cliente in clientes.data" :key="cliente.id">
                              ...
                              <td class="text-dark-50 text-hover-primary">{{cliente.description}}</td>
          ...
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title">
                {{ editMode ? "Agregar" : "Editar" }} tipo de rubro
              </h5>
              <button
                type="button"
                class="close"
  ...
  export default {
    ...
    methods: {
      getClientes(page = 1,table = "name") {
        ...
        axios.get("/api/tipo-rubros?page=" + page + "&paginate=" + this.paginate + "&q=" + this.search + "&sort=" + sort + "&table=" + table).then((response) => {
      ...
      getCliente(id) {
        axios.get("/api/tipo-rubros/" + id).then((response) => {
      ...
      delete_cliente(id, name) {
        this.$swal({
          ...
          if (result.value) {
            axios
              .delete("/api/tipo-rubros/" + id+"?id_usuario="+user.id)
              .then((response) => {
                /*
                this.$swal({
                  icon: "success",
      ...
      updateCliente() {
        this.clientFormLoading = true;
        axios
          .put("/api/tipo-rubros/" + this.idCliente+"?id_usuario="+user.id, this.form)
          .then((response) => {
            $("#createModal").modal("hide");
            this.clientFormLoading = false;
            toastr.success('Tipo de Rubro Actualizado');
            //this.$swal({ icon: "success", title: "Tipo de Rubro Actualizado" });
            this.getClientes();
          })
          .catch((error) => {
      ...
      crearCliente() {
        this.clientFormLoading = true;
        axios
          .post("/api/tipo-rubros"+"?id_usuario="+user.id, this.form)
          .then((response) => {
            $("#createModal").modal("hide");
            this.clientFormLoading = false;
            //this.$swal({ icon: "success", title: "Tipo de Rubro Guardado" });
            toastr.success('Tipo de Rubro Guardado');
            this.getClientes();
          })
          .catch((error) => {
            if (error.response.status === 422) {
