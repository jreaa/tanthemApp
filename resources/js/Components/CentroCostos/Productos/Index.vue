    ...
    margin-left: auto!important;"
                    ...
                    >
                        ...
                        <tr role="row">
                            ...
                            @click="changeSort(p,'p1')"
                            >
                             <div style="display: flex;justify-content: space-between;">
                             Nombre
                              <div>
                                  <i v-if="sort && sortingvalue == 'p1'" class="flaticon2-arrow-down" style="font-size: 0.6rem;color: #1BC5BD;"></i>
                                  <i v-if="sort == false && sortingvalue == 'p1'" class="flaticon2-arrow-up" style="font-size: 0.6rem;color: #1BC5BD;"></i>
                              </div>
                              </div>
                            </th>
                            <th
                        ...
                        <tbody>
                          ...
                          <tr v-if="registro == true">
                            <td><input v-bind:class="errors.p1 ? ' is-invalid' : ''" type="text" v-model="form.p1" class="form-control" placeholder="Ej: Producto A"></td>
                        ...
                        <tr class="even" v-for="producto in productos.data" :key="producto.id">
                            ...
                            <td>{{producto.carac}}</td>
                            <td>{{producto.costo}}</td>
                            <td>{{producto.pv}}</td>
                            <td v-if="formN.n1" class="td-n">{{producto.n1}}</td>
                            <td v-if="formN.n2" class="td-n">{{producto.n2}}</td>
                            <td v-if="formN.n3" class="td-n">{{producto.n3}}</td>
                            <td v-if="formN.n4" class="td-n">{{producto.n4}}</td>
                            <td v-if="btnEstructura">&nbsp;</td>
                            <td nowrap="nowrap" style="">
                            <button class="btn btn-sm btn-clean btn-icon" v-if="productosEdit" @click="updateModal(producto.id)">
                                    <i class="la la-edit"></i>
                            </button>
                            <button @click="delete_producto(producto.id, producto.name)" v-if="productosDelete" class="btn btn-sm btn-clean btn-icon">
...
methods: {
    ...
    crearProducto() {
        ...
        axios
        .post("/api/productos", this.form)
        .then((response) => {
            this.clientFormLoading = false;
            this.registro = false;
            this.$swal({ icon: "success", title: "Producto Guardado" });
            this.getProductos();
        })
        .catch((error) => {
    ...
    updateProducto() {
        this.clientFormLoading = true;
        axios
            .put("/api/productos/" + this.idCliente, this.form)
            .then((response) => {
            $("#createModal").modal("hide");
            this.clientFormLoading = false;
            this.registro = false;
            this.$swal({ icon: "success", title: "Producto Actualizado" });
            this.getProductos();
            })
            .catch((error) => {
            if (error.response.status === 422) {
...
.btnValue {
    ...
    width: 70%;color: #B5B5C3 !important;
