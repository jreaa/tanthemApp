<template>
    <div>
      <div style="margin-bottom:50px!important" class="d-flex align-items-center font-weight-bold my-2 mb-4" bis_skin_checked="1">
          ...
          <a href="/" style="font-size: 13!important" class="opacity-75 hover-opacity-100" bis_skin_checked="1">Estados Producto</a>
          <!--end::Item-->
      </div>
      <h3 class="mt-4">Tipo Estados Proyectos</h3>
      <br />
      <div class="card card-custom">
        <div class="card-header flex-wrap py-5">
          <div class="card-title">
            <h3 class="card-label">
              Datos de los estados de proyectos asociado
              <span class="d-block text-muted pt-2 font-size-sm"
                >A continuacion podra observar y gestionar cada uno de sus tipo de
                estados de productos</span
              >
            </h3>
          </div>
      ...
      margin-left: auto!important;"
                      ...
                      <div class="alert-text">
                          No hay estados de proyectos registrados 
                      </div>
                      </div>
  ...
  export default {
    ...
    methods: {
      getEstados(page = 1,table = "name") {
        ...
        axios.get("/api/estados-proyectos?page=" + page + "&paginate=" + this.paginate + "&q=" + this.search + "&sort=" + sort + "&table=" + table).then((response) => {
      ...
      getCliente(id) {
        axios.get("/api/estados-proyectos/" + id).then((response) => {
      ...
      delete_cliente(id, name) {
        this.$swal({
          ...
          if (result.value) {
            axios
              .delete("/api/estados-proyectos/" + id+"?id_usuario="+user.id)
              .then((response) => {
                /*
                this.$swal({
                  icon: "success",
      ...
      updateEstado() {
        ...
        console.log(user.id)
        axios
          .put("/api/estados-proyectos/" + this.idCliente+"?id_usuario="+user.id, this.form)
          .then((response) => {
      ...
      crearEstado() {
        this.clientFormLoading = true;
        axios
          .post("/api/estados-proyectos"+"?id_usuario="+user.id, this.form)
          .then((response) => {
            $("#createModal").modal("hide");
            this.clientFormLoading = false;
             this.registro = false;
