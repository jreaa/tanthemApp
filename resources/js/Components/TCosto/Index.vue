<template>
  <div>
    <div style="margin-bottom:50px!important" class="d-flex align-items-center font-weight-bold my-2 mb-4" bis_skin_checked="1">
        <!--begin::Item-->
        <router-link
                :to="{ name: 'tcostos.index' }"  class="opacity-75 hover-opacity-100" bis_skin_checked="1">
          <i class="flaticon2-shelter"></i>
        </router-link>
        <!--end::Item-->
        <!--begin::Item-->
        <span class="label label-dot label-sm bg-black opacity-75 mx-3"></span>
        <a href="/" style="font-size: 13!important" class="opacity-75 hover-opacity-100" bis_skin_checked="1">Tipo Costos</a>
        <!--end::Item-->
    </div>
    <h3 class="mt-4">Tipo Costos</h3>
    <br />
    <div class="card card-custom">
      <div class="card-header flex-wrap py-5">
        <div class="card-title">
          <h3 class="card-label">
            Datos de los tipo costos Asociado
    ...
    margin-left: auto!important;"
                    ...
                    <div class="alert-text">
                        No hay tipo costos registrados 
                    </div>
                    </div>
                    ...
                    >
                        ...
                        <tbody>
                          ...
                          <tr v-if="registro == true">
                            <td><input v-bind:class="errors.name ? ' is-invalid' : ''" type="text" v-model="form.name" class="form-control" placeholder="Ej: Activo"></td>
                            <td>
                              <input v-bind:class="errors.color ? ' is-invalid' : ''" type="color" v-model="form.color" class="form-control"  placeholder="#fff" style="width: 100%!important;">
                             
                            </td>
                            <td>
                               <input v-bind:class="errors.colorFont ? ' is-invalid' : ''" type="color" v-model="form.colorFont" class="form-control"  placeholder="#fff" style="width: 100%!important;">
                            </td>
                            ...
                            <td><input v-bind:class="errors.description ? ' is-invalid' : ''" type="text" v-model="form.description" class="form-control" placeholder="Color verde"></td>
                        ...
                        <tr class="even" v-for="estado in estados.data" :key="estado.id">
                            ...
                            <td v-bind:title="estado.description">
                                    <div v-if="estado.description.length<8">{{estado.description}}</div>
                                    <div v-else> {{ estado.description.substring(0,8)+".." }}</div>
                            </td>
...
export default {
  ...
  methods: {
    getEstados(page = 1,table = "name") {
      ...
      axios.get("/api/tcostos?page=" + page + "&paginate=" + this.paginate + "&q=" + this.search + "&sort=" + sort + "&table=" + table).then((response) => {
    ...
    getCliente(id) {
      axios.get("/api/tcostos/" + id).then((response) => {
    ...
    updateEstado() {
      this.clientFormLoading = true;
      axios
        .put("/api/tcostos/" + this.idCliente, this.form)
        .then((response) => {
